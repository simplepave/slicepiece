...
class ControllerNewsBlogArticle extends Controller {
	...
	public function index() {
        ...
        $images_size_articles_big     = [600, 400];
        $images_size_articles_small   = [400, 200];
        $images_size_articles_related = [400, 200];
		...
		if (isset($this->request->get['newsblog_path'])) {
			...
			if ($category_info) {
				...
				if ($category_info['settings']) {
		            ...
		            $images_size_articles_big = [
                        (int)$settings['images_size_articles_big_width']?: 600,
                        (int)$settings['images_size_articles_big_height']?: 400,
                    ];

		            $images_size_articles_small = [
                        (int)$settings['images_size_articles_small_width']?: 400,
                        (int)$settings['images_size_articles_small_height']?: 200,
                    ];
	            }
			}
		}
		...
		if ($article_info) {
			...
			if ($article_info['image']) {
				$data['original']	= $scheme . 'image/'.$article_info['image'];
			...
			foreach ($results as $result) {
				$data['images'][] = array(
					'original'	=> $scheme . 'image/'.$result['image'],
